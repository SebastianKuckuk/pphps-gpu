...
int main(int argc, char *argv[]) {
    ...
    #pragma omp target enter data map(alloc: u[0:nx*ny], uNew[0:nx*ny])
    ...
    double res = 0;
    #pragma omp target loop collapse(2) reduction(+ : res)
    for (size_t j = 1; j < ny - 1; ++j) {
        for (size_t i = 1; i < nx - 1; ++i) {
    ...
    #pragma omp target exit data map(release: u[0:nx*ny], uNew[0:nx*ny])
